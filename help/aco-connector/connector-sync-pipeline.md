...
1. **Detección de cambio de entidad** — (cada 1 min) Un trabajo cron (`indexer_reindex_all_invalid`) detecta [!DNL Adobe Commerce] cambios de entidad y déclencheur el [!DNL SaaS Data Export], que ensambla los elementos de fuente y realiza un seguimiento de su estado.
...
Dos grupos de cron automatizan la canalización en un horario fijo.

| Grupo Cron | Finalidad | Programación |
| ---------- | ------- | -------- |
| `indexer_reindex_all_invalid` | Escucha actualizaciones de entidad, ensambla elementos de fuente y mantiene el estado de la fuente | Cada 1 minuto |
| `*_resend_failed_items` | Comprueba los elementos de fuente con errores y los vuelve a enviar a [!DNL Commerce Optimizer] | Cada 5 minutos |
...
- Los indexadores de fuente deben utilizar el modo **[!UICONTROL Update by Schedule]**. Consulte [Comprobar la configuración de la aplicación Commerce](../data-export/data-synchronization.md#verify-commerce-application-configuration){target="_blank"}.
