...
La instancia de Commerce se conecta mediante una clave de API y una clave privada, y se especifica el espacio de datos en la configuración mediante [Commerce Services Connector](../landing/saas.md). **Solo configuró esta conexión una vez.**
...
Si se pierde su clave de API, se debe [generar](../landing/saas.md#genapikey) un nuevo par de claves de API y [aplicar](../landing/saas.md#createsaasenv) a la configuración del Conector de servicios de Commerce en el administrador. Si se configuran claves incorrectas o no existe ninguna en la configuración, aparecerá un cuadro de diálogo de error de verificación de cuenta en Payment Services que le notificará que la cuenta no se ha verificado.

Ver una [lista de servicios de Commerce disponibles que usan la API](../landing/saas.md#availableservices).

Para obtener información sobre cómo generar una clave de API para entornos de zona protegida o de producción, consulte [Credenciales](../landing/saas.md#apikey).
...
La misma clave de API se puede usar en todas las instancias, pero cada una debe tener su propio [espacio de datos SaaS](../landing/saas.md#saasenv).
...
1. Para configurar los servicios de Commerce, siga los pasos descritos en [Configuración de SaaS](../landing/saas.md#saasenv).
...
[!DNL Payment Services] utiliza [Commerce Services Connector](../landing/saas.md) para conectarse a Commerce Services e implementarlo como SaaS. Este(a) [!DNL Commerce Services Connector] se comunica a través del extremo en:
