...
1. Los valores de **[!UICONTROL Payment Services Sandbox ID]** y **[!UICONTROL Payment Services Production ID]** se rellenan automáticamente una vez que configura [Commerce Services Connector](../landing/saas.md){target=_blank} y visita el panel [!DNL Payment Services] por primera vez. Haga esto para finalizar la incorporación para su zona protegida o entornos de producción. Estos valores asocian su ID de SaaS a [!DNL Payment Services].
...
En versiones anteriores, la asignación de cuentas de PayPal a nivel de sitio web requería que [contactaras con el servicio de asistencia](https://experienceleague.adobe.com/es/docs/support-resources/adobe-support-tools-guide/adobe-commerce-support/adobe-commerce-help-center-user-guide#support-case) o con tu representante de Adobe. Utilice la acción **[!UICONTROL Connect different account]** cuando cumpla los requisitos previos siguientes.
...
1. La instalación de [Commerce Services Connector](../landing/saas.md) se ha completado.

1. [La zona protegida y las claves de API de producción](connect.md#configure-commerce-services) (públicas y privadas) se guardan en el administrador.

1. **[!UICONTROL Payment Services Sandbox ID]** y **[!UICONTROL Payment Services Production ID]** se han rellenado en [Configuración general](#general-configuration).
...
Consulte [Opciones de pago](payments-options.md#paypal-payment-buttons) para obtener más información.
...
| [!UICONTROL Payment Action] | sitio web | La [acción de pago](https://experienceleague.adobe.com/es/docs/commerce-admin/config/sales/payment-methods/payment-methods) para el método de pago especificado. Opciones: [!UICONTROL Authorize] / [!UICONTROL Authorize and Capture] |
...
Con [!DNL Apple Pay], los comerciantes pueden ofrecer una experiencia de pago segura, rápida y sin problemas, que admite hasta 99 dominios por cuenta de comerciante. En **Safari** (macOS y iOS), el botón [!DNL Apple Pay] rellena automáticamente la información de pago, contacto y envío desde el dispositivo del cliente, tanto al principio del cierre de compra (express) como en la página final de cierre de compra. En **Chrome, Firefox o Microsoft Edge**, [!DNL Apple Pay] está disponible durante el **pago y envío exprés** y en el **paso final de pago**; en el escritorio, un código QR y un **iPhone** (iOS 18 o posterior) permiten al comprador finalizar el pago en la hoja de pago de Apple. Asegúrese de que **[!UICONTROL Show Apple Pay on product detail page]** u otras ubicaciones estén habilitadas donde desee realizar la desprotección rápida.

>[!IMPORTANT]
>
> Apple rechaza los pagos de dominios no verificados. Los comerciantes deben verificar cualquier dominio donde se utilicen botones de Apple Pay. Existen restricciones de país. Consulta la [documentación de Apple Pay](https://developer.apple.com/documentation/apple_pay_on_the_web) para obtener más información sobre los requisitos de verificación de dominio.
...
1. En el panel izquierdo, expanda **[!UICONTROL Sales]** y elija **[!UICONTROL Payment Methods]**.
1. Expanda la sección _[!UICONTROL FEATURED ADOBE PAYMENT SOLUTION]_.
1. En la sección _[!UICONTROL [!DNL Payment Services]]_, expanda la sección&#x200B;_[!UICONTROL Apple Pay]_.
1. Para **[!UICONTROL Title]**, escriba texto (si es necesario) para cambiar el nombre del método de pago como se muestra durante el cierre de compra.
...
| [!UICONTROL Title] | vista de tienda | Añada el texto que se mostrará como título para esta opción de pago en la vista Método de pago durante el cierre de compra. Opciones: [!UICONTROL text field] |
| [!UICONTROL Payment Action] | sitio web | La [acción de pago](https://experienceleague.adobe.com/es/docs/commerce-admin/config/sales/payment-methods/payment-methods) para el método de pago especificado. Opciones: [!UICONTROL Authorize] / [!UICONTROL Authorize and Capture] |
| [!UICONTROL Show Apple Pay on checkout page] | sitio web | Habilitar o deshabilitar [!DNL Apple Pay] en la página de cierre de compra. Opciones: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Show Apple Pay at start of checkout] | vista de tienda | Habilitar o deshabilitar [!DNL Apple Pay] al inicio del flujo de cierre de compra. Opciones: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Sort order] | vista de tienda | El criterio de ordenación para el método de pago especificado en la página de pago. Valor `Numeric Only` |
| [!UICONTROL Show Apple Pay on product detail page] | vista de tienda | Habilite o deshabilite [!DNL Apple Pay] en la página de detalles del producto. Opciones: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Show Apple Pay in mini cart preview] | vista de tienda | Habilite o deshabilite [!DNL Apple Pay] en la vista previa del minicarrito. Opciones: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Show Apple Pay on cart page] | vista de tienda | Habilite o deshabilite [!DNL Apple Pay] en la página del carro de compras. Opciones: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Debug Mode] | sitio web | Habilite o deshabilite el modo de depuración. Opciones: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
...
1. En el panel izquierdo, expanda **[!UICONTROL Sales]** y elija **[!UICONTROL Payment Methods]**.
1. Expanda la sección _[!UICONTROL FEATURED ADOBE PAYMENT SOLUTION]_.
1. En la sección _[!UICONTROL [!DNL Payment Services]]_, expanda la sección&#x200B;_[!UICONTROL Google Pay]_.
...
1. Para elegir si los compradores ven una página de **revisión de Google Pay** independiente después de la hoja de pago de Google Pay, establece **[!UICONTROL Skip Review]** en `Yes` o `No`. Cuando se establece en `Yes`, los flujos urgentes admitidos muestran **métodos de envío en la hoja Google Pay** (devoluciones de llamadas de envío del lado del cliente) y pueden completarse sin el paso de revisión adicional. Cuando se establece en `No`, los compradores pueden confirmar el envío y los totales en la página de revisión antes de pagar.
1. Para habilitar **[!UICONTROL 3D Secure authentication]** (`Off` de forma predeterminada), elija `Always` o `When required`.
1. Para habilitar el modo de depuración, seleccione `Yes` para **[!UICONTROL Debug Mode]** (`No` lo deshabilita).
...
1. Para establecer la altura, utilice el valor predeterminado para la altura definida en **[!UICONTROL Button Style]**.
1. Para guardar los cambios, haga clic en **[!UICONTROL Save Config]**
1. Vaya a **[!UICONTROL System]** > **[!UICONTROL Cache Management]** y, a continuación, haga clic en **[!UICONTROL Flush Cache]** para actualizar todas las cachés no válidas.
...
| [!UICONTROL Payment Action] | sitio web | La [acción de pago](https://experienceleague.adobe.com/es/docs/commerce-admin/config/sales/payment-methods/payment-methods) para el método de pago especificado. Opciones: `[!UICONTROL Authorize]` / `[!UICONTROL Authorize and Capture]` |
| [!UICONTROL Show Google Pay on checkout page] | sitio web | Habilitar o deshabilitar [!DNL Google Pay] en la página de cierre de compra. Opciones: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Show Google Pay at start of checkout] | vista de tienda | Habilitar o deshabilitar [!DNL Google Pay] al inicio del flujo de cierre de compra. Opciones: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Sort order] | vista de tienda | El criterio de ordenación para el método de pago especificado en la página de pago. Valor `Numeric Only` |
| [!UICONTROL Show Google Pay on product detail page] | vista de tienda | Habilite o deshabilite [!DNL Google Pay] en la página de detalles del producto. Opciones: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Show Google Pay in mini cart preview] | vista de tienda | Habilite o deshabilite [!DNL Google Pay] en la vista previa del minicarrito. Opciones: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Show Google Pay on cart page] | vista de tienda | Habilite o deshabilite [!DNL Google Pay] en la página del carro de compras. Opciones: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Skip Review] | vista de tienda | Cuando se establece en `[!UICONTROL Yes]`, los flujos urgentes de [!DNL Google Pay] aptos pueden omitir la página de revisión independiente después de la hoja de pago; los métodos de envío aparecen en la hoja de Google Pay. Cuando se establece en `[!UICONTROL No]`, los compradores pasan a la página de revisión para confirmar el envío y los totales. Opciones: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
...
| [!UICONTROL Display Pay Later Message] | sitio web | Activa o desactiva la mensajería PayPal Más tarde en el carro de compras, la página de productos, el minicarrito y durante el flujo de pago. Opciones: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
...
| [!UICONTROL Show buttons at start of checkout] | vista de tienda | Habilitar o deshabilitar [!DNL PayPal payment buttons] al inicio del flujo de cierre de compra. Opciones: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Sort order] | vista de tienda | El criterio de ordenación para el método de pago especificado en la página de pago. Valor `Numeric Only` |
...
| [!UICONTROL App Switch] | vista de tienda | Esta opción solo se aplica a los comerciantes y compradores de **US**. Cuando estén activados, todos los pagos Express PayPal intentarán pasar a través de la aplicación móvil PayPal cuando estén disponibles. Opciones: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Contact Preference] | vista de tienda | Esta opción solo se aplica a los comerciantes y compradores de **US**. Si se establece en **yes**, el correo electrónico y el número de teléfono de los compradores aparecerán en el modo de pago de PayPal. Opciones: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
...
1. Para **[!UICONTROL Sort Order]**, introduzca un valor numérico para determinar la posición de visualización de las LPM dentro de la lista de métodos de pago disponibles.
1. Para guardar los cambios, haga clic en **[!UICONTROL Save Config]**.
1. Vaya a **[!UICONTROL System]** > **[!UICONTROL Cache Management]** y, a continuación, haga clic en **[!UICONTROL Flush Cache]** para actualizar todas las cachés no válidas.
...
| Campo | Ámbito | Descripción |
|---|---|---|
| [!UICONTROL Active] | sitio web | Habilite o deshabilite los métodos de pago locales. Opciones: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Title] | vista de tienda | El nombre del método de pago mostrado durante el cierre de compra y en la cuadrícula del pedido de ventas. Opciones: [!UICONTROL text field] |
| [!UICONTROL Allowed Payment Methods] | sitio web | Seleccione los LPM que desea ofrecer. Los métodos solo son visibles para los clientes cuya dirección de facturación y moneda del sitio web coinciden con los requisitos del método. Opciones: `Bancontact` / `BLIK` / `eps` / `iDEAL` / `MyBank` / `Przelewy24` |
| [!UICONTROL Sort Order] | vista de tienda | La posición de visualización del grupo LPM dentro de la lista de métodos de pago disponibles. Esta configuración se aplica a todos los LPM como grupo; los LPM individuales no se pueden ordenar de forma independiente. Valor `Numeric Only` |
...
Los artículos de línea envían información detallada del pedido a PayPal, incluidos detalles del producto, cantidades y precios. Esta función está habilitada de forma predeterminada. Consulte [Elementos de línea](line-items.md) para obtener más información.

### Opciones de configuración

| Campo | Ámbito | Descripción |
|---|---|---|
| [!UICONTROL Line Items Enabled] | sitio web | Activa o desactiva el envío de artículos de línea y el desglose de importes a PayPal. Deshabilite esta configuración si usa extensiones de terceros que agregan tarifas personalizadas no admitidas por [!DNL Payment Services]. Opciones: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
...
Consulte [Funciones de usuario](https://experienceleague.adobe.com/es/docs/commerce-admin/systems/user-accounts/permissions-user-roles) para obtener información sobre cómo administrar las funciones.
...
Vea [Sitio, tienda y ámbito de vista](https://experienceleague.adobe.com/es/docs/commerce-admin/start/setup/websites-stores-views) para obtener más información sobre la jerarquía de sitios web, tiendas y vistas de tiendas.
