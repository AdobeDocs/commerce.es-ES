...
## Alineación del entorno de integración de ACO {#aco-integration-environment-alignment}
...
>[!NOTE]
>
>Para las soluciones de Commerce que usan Adobe Commerce Optimizer o el conector de Adobe Commerce Optimizer, usa la [API de GraphQL de servicios de comercialización](https://developer.adobe.com/commerce/services/optimizer/merchandising-services/using-the-api/) en lugar de la API de GraphQL del servicio de catálogo.

## Comprobación de sincronización de datos para Optimizer {#aco-data-sync-verification}
...
## Nota desplegable de ACO para actualizaciones de la API {#aco-api-updates-and-dropins}

>[!NOTE]
>
>[Los componentes integrados](https://experienceleague.adobe.com/developer/commerce/storefront/dropins/all/introduction/) de [!DNL Commerce Storefront on Edge Delivery Services] recopilan automáticamente los cambios más recientes de GraphQL (nuevos campos, límites y comportamiento de las consultas).
...
## Función experimental solo de zona protegida ACCS {#accs-sandbox-experimental}

>[!IMPORTANT]
>
>Esta característica es experimental y solo está disponible en entornos de espacio aislado de [!DNL Adobe Commerce as a Cloud Service].
>
...
## Asignación de instancias de AEM Assets {#aem-assets-instance-mapping}

>[!NOTE]
>
>Al conectar [!DNL Adobe Commerce as a Cloud Service] a [!DNL AEM Assets], la instancia de ensayo [!DNL AEM Assets] se asigna a la instancia de zona protegida [!DNL Adobe Commerce as a Cloud Service] y a cualquier otro entorno que no sea de producción. La instancia de producción [!DNL AEM Assets] se asigna a la instancia de producción [!DNL Adobe Commerce as a Cloud Service].

## Identidad de IMS e información de inicio de sesión único {#ims-identity-and-sso-config}
