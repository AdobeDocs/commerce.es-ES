...
Cuando se trata de buscar sitios, Adobe Commerce le da opciones. Para asegurarse de que [!DNL Live Search] y [!DNL Catalog Service] satisfacen las necesidades de su empresa, revise los límites y límites siguientes. Si necesita funcionalidades de búsqueda avanzadas como búsqueda de contenido, traer su propio algoritmo (BYOA) o comercialización basada en atributos, considere una solución de búsqueda de terceros.

## General

- El adaptador de búsqueda ha sido [obsoleto](release-notes.md#live-search-400) desde [!DNL Live Search] 4.0.0. El widget de página de lista de productos (PLP) es la solución compatible para todas las implementaciones de [!DNL Live Search] en adelante. El adaptador de búsqueda solo recibirá actualizaciones relacionadas con la seguridad. Consulte la [guía de migración](migrate-to-plp.md) para obtener información sobre cómo migrar al widget PLP.
...
- [!DNL Live Search] [índices](indexing.md) hasta un total de 450 atributos de producto por vista de tienda. Estos atributos se distribuyen de la siguiente manera:
...
- El estado de las existencias (`quantity_and_stock_status`) no se admite como faceta. En el Administrador, puede establecer [!UICONTROL Display Out of Stock Products] = [!UICONTROL No] para filtrar los productos sin existencias. Esta funcionalidad se admite de forma nativa en el módulo `[PLP Widget](plp-styling.md)`. Para obtener más información, consulte [Administrar productos sin existencias](manage-out-of-stock-products.md).
...
- Las reglas y los productos clasificados manualmente se aplican a los resultados de búsqueda cuando se selecciona el orden predeterminado, &quot;Ordenar por: Más relevante&quot;. Si un comprador cambia el criterio de ordenación, las reglas y las clasificaciones manuales ya no están en vigor.
...
- [[!DNL popover]](storefront-popover.md) solo está disponible para tiendas que usen el tema *Luma* o un tema personalizado basado en *Luma*. A las rutas de exploración de la página de resultados de búsqueda les falta el estilo *Luma*.
...
- [[!DNL Live Search] catálogo no sincronizado](https://experienceleague.adobe.com/en/docs/commerce-knowledge-base/kb/troubleshooting/miscellaneous/live-search-catalog-data-sync): proporciona soluciones para los casos en los que los datos del catálogo de productos no se sincronizan correctamente entre la tienda Adobe Commerce y el servicio Live Search. Este artículo explica cómo comprobar el estado de sincronización, identificar errores de sincronización y resolver problemas de sincronización de datos.
- [[!DNL Live Search] La clasificación del tablero y los resultados de búsqueda es incorrecta](https://experienceleague.adobe.com/en/docs/commerce-knowledge-base/kb/troubleshooting/miscellaneous/live-search-dashboard-ranking-incorrect): soluciona problemas en los que los resultados de búsqueda o las métricas de rendimiento mostrados en el tablero de Live Search no se muestran según lo esperado. Este artículo explica cómo solucionar discrepancias de clasificación e incoherencias en los datos del panel.
...
Si necesita ayuda adicional, comuníquese con [soporte técnico](https://experienceleague.adobe.com/en/docs/support-resources/adobe-support-tools-guide/adobe-commerce-support/adobe-commerce-help-center-user-guide).
