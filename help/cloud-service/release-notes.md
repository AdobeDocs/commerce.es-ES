...
Los siguientes elementos se publicaron en el entorno de producción el 8 de septiembre de 2026.
...
Los nuevos puntos de conexión de API REST [`GET` y `PUT /V1/system/config`](https://developer.adobe.com/commerce/webapi/rest/saas-integrations/system-config) permiten que las integraciones lean y actualicen los valores de configuración del sistema de Commerce, lo que incluye:
...
Una nueva consulta de GraphQL [`sourceAvailability`](https://developer.adobe.com/commerce/webapi/graphql/schema/products/queries/source-availability) devuelve la disponibilidad de existencias por origen para uno o más SKU, de modo que las tiendas, como las páginas de productos y categorías, pueden mostrar información de existencias precisa para cada origen de inventario.

[Habilitar **disponibilidad por Source**](https://experienceleague.adobe.com/es/docs/commerce-admin/inventory/configuration/global-options). <!-- ACCS-933 -->

### Leer la configuración de uso compartido de cuentas y la lista de deseos persistentes a través de GraphQL

La consulta de GraphQL [`storeConfig`](https://developer.adobe.com/commerce/webapi/graphql/schema/store/queries/store-config/#query-a-stores-persistent-cart-and-account-sharing-configuration) ahora devuelve los valores de configuración `persistent_enabled`, `persistent_shopping_cart`, `persistent_options_wishlist` y `share_customer_accounts_scope`, de modo que los escaparates puedan acceder a la configuración de la lista de deseos y el carro de compras persistente del comerciante sin ponerse en contacto con el soporte técnico. <!-- USF-4051 -->

### Buscar pedidos de clientes por producto, SKU o ID de pedido

La entrada de GraphQL [`CustomerOrdersFilterInput`](https://developer.adobe.com/commerce/webapi/graphql/schema/customer/queries/customer) ahora admite un campo `search` opcional que coincide con el número de pedido, el SKU de artículo o el nombre de artículo, combinado con cualquier otro filtro que proporcione. <!-- USF-4290 -->
...
| --- | --- | --- |
| `POST` | `/V1/orders/{orderId}/edit/start` | Copie el pedido en un nuevo carro de compras editable y devuelva el ID del carro de compras. |
| `POST` | `/V1/orders/{orderId}/edit/submit` | Envíe el carro de compras modificado como un pedido nuevo y cancele el pedido original. |

Después de llamar a `edit/start`, modifique el carro devuelto usando los extremos REST estándar del carro y llame a `edit/submit`. El nuevo pedido hereda el método de pago del pedido original a menos que lo sustituya a través del carro de compras y se cree como un reemplazo vinculado para el original cancelado. Ambos extremos requieren el recurso ACL `Magento_Sales::actions_edit`. <!-- ACCS-1284 -->
...
### Importar más códigos de cupones por archivo

El límite máximo de importación de cupones en bloque por archivo se puede ajustar poniéndose en contacto con el administrador de éxito del cliente de Adobe Commerce o creando un ticket de asistencia. <!-- CCSAAS-5176 -->

### Administrar plantillas de correo electrónico personalizadas mediante la API

Los siguientes nuevos extremos de API de REST permiten que las integraciones incluyan, recuperen y creen [plantillas de correo electrónico personalizadas](https://developer.adobe.com/commerce/webapi/rest/saas-integrations/custom-email/):

| Método | Extremo | Descripción |
| --- | --- | --- |
| `GET` | `/V1/custom-email/templates` | Enumere sus plantillas de correo electrónico personalizadas y devuelva el ID, el código, el asunto y el tipo de cada plantilla. |
| `GET` | `/V1/custom-email/templates/{id}` | Recupere una sola plantilla, incluidos el cuerpo y los estilos. |
| `POST` | `/V1/custom-email/templates` | Cree una plantilla de correo electrónico personalizada y devuelva su ID asignado por el servidor. |

Use un identificador de plantilla devuelto con el extremo `POST /V1/custom-email/send` en lugar de buscar el identificador manualmente.

Todos los extremos de `custom-email` requieren acceso al `Marketing > Communications > Email template` [recurso de rol](https://experienceleague.adobe.com/es/docs/commerce-admin/systems/user-accounts/permissions-user-roles#step-2assign-resources). <!-- CCSAAS-5089, CCSAAS-5090 -->

### Administrar la secuencia de orden completa mediante la API de REST
...
Los extremos de `GET` que admiten el filtrado en facturas, envíos, notas de abono y devoluciones ahora admiten el filtrado por `order_original_id`. Filtrar por `order_original_id` devuelve detalles sobre toda la cadena de pedidos, no solo el pedido único. Un extremo de ejemplo que admite esta característica es `GET /V1/invoices`. <!-- ACCS-1004, ACCS-1005 -->

### Buscar en la cuadrícula de pedidos por valores de atributo personalizados

>[!IMPORTANT]
>
>Esta función está desactivada de forma predeterminada. Para habilitarlo, póngase en contacto con el administrador de satisfacción del cliente de Adobe Commerce o cree un ticket de asistencia.

Los comerciantes ahora pueden filtrar la cuadrícula de pedidos [!DNL Commerce Admin] según los valores almacenados en atributos personalizados de pedidos. Hay un filtro de [!UICONTROL **Atributos personalizados**] disponible en la fila de filtro de cuadrícula de orden.<!-- ACCS-923 -->

### Establecer un origen de inventario designado en los artículos del carro de compras

La nueva mutación de GraphQL `setNominatedSourceOnCartItems` asigna un origen de inventario específico a los artículos del carro de compras, lo que admite escenarios como la recogida en la tienda (BOPIS) y el envío desde la tienda. La mutación acepta un(a) `cart_id` y una lista de elementos, cada uno con un(a) `cart_item_uid` y un(a) `source_code`, y devuelve cualquier(a) `rejected_items` con un código de error estructurado: `UNKNOWN_SOURCE`, `SOURCE_DISABLED`, `NOT_ENOUGH_QTY` o `SKU_SOURCE_CONFLICT`. Cada SKU de un carro de compras se resuelve en un único origen designado, y al pasar un `source_code` nulo o vacío, se borra la nominación. <!-- ACCS-932 -->

### Suscribirse a un evento para que los carros de compras coincidan con las reglas de recordatorio

Se emite un nuevo evento `observer.reminder_matched_carts` después de que las reglas de recordatorio de correo electrónico ejecuten su lógica correspondiente y lleven información sobre los carros de compras que coinciden. Las integraciones pueden suscribirse a este evento y reenviar los datos a un sistema externo, como una plataforma de marketing, en lugar de depender de los correos electrónicos de recordatorio nativos. <!-- CCSAAS-5173 -->

### Suprimir correos electrónicos transaccionales por área o plantilla

Una nueva configuración de [Supresión de correo electrónico](https://experienceleague.adobe.com/es/docs/commerce-admin/config/services/email-suppression) ([!UICONTROL **Tiendas**] > [!UICONTROL **Configuración**] > [!UICONTROL **Servicios de Adobe**] > [!UICONTROL **Supresión de correo electrónico**]) permite a los administradores detener selectivamente [!DNL Commerce] de enviar correos electrónicos transaccionales. Puede suprimir correos electrónicos por área funcional (cuenta de cliente, Order Management, devoluciones, cierre de compra, marketing o B2B) o por una lista exacta de identificadores de plantilla.<!-- ACCS-1025 -->

### Ver el historial de modificaciones de pedidos en el administrador

La página de detalles de pedido [!DNL Commerce Admin] ahora muestra la secuencia de modificación completa de un pedido que incluye el pedido original y todos los pedidos secundarios creados mediante ediciones posteriores. Los comerciantes pueden desplazarse entre pedidos, cambiar la visibilidad de los pedidos cancelados y acceder a todas las facturas, envíos, notas de abono y comentarios de pedidos asociados desde la vista de secuencia.<!-- ACCS-968 -->
...
* Se mejoró el rendimiento de la integración [!DNL AEM Assets]. <!-- ACAP-1242 -->

* Se ha corregido un error que se podía producir al agregar un SKU de producto simple a un producto configurable en [!DNL Commerce Admin]. <!-- ACCS-1132 -->

* Se ha corregido un problema en el cual la cola de mensajes podía dejar de procesar nuevos mensajes cuando acumulaba demasiados registros obsoletos. <!-- ACCS-1292 -->

* Se ha corregido un problema en el cual la creación del pedido de administración fallaba con un error de &quot;SKU no disponible en el catálogo compartido&quot;. <!-- ACCS-1318 -->

* Se ha resuelto un bloqueo que se producía al crear o editar productos agrupados. <!-- CCSAAS-5211 -->

* Se ha corregido un problema en el cual la colocación de pedidos no reservaba inventario en el origen designado para artículos que usaban recogida en tienda o envío desde tienda. <!-- ACCS-1374 -->

* Las tarifas personalizadas antiguas ahora se eliminan de la respuesta de consulta del carro de compras. <!-- ACCS-1400 -->

* Se ha resuelto un problema en la integración de [!DNL AEM Assets] en el cual los atributos de función de recurso de producto perdían datos de configuración regional durante la exportación del catálogo. <!-- ACCS-1401 -->

* Se mejoró la advertencia recibida al guardar una integración que indica que [!DNL Dynamic Media] no está habilitado. <!-- ACAP-1298 -->

* Los campos Nombre del evento y Alias ahora se validan para que aparezcan en minúsculas al suscribirse a un evento. <!-- CEXT-6164 -->
...
### Omitir reCAPTCHA para la autenticación OTP mediante programación

Una nueva opción de configuración le permite omitir la validación de reCAPTCHA para la mutación de GraphQL [`exchangeOtpForCustomerToken`](https://developer.adobe.com/commerce/webapi/graphql/schema/customer/mutations/exchange-otp-customer-token). Esto permite flujos de trabajo de punchout B2B. El intercambio de contraseña única (OTP) se inicia mediante programación sin una entrada de formulario, lo que hace innecesaria la validación de reCAPTCHA. Esta capacidad se basa en la característica [inicio de sesión con código único](https://experienceleague.adobe.com/es/docs/commerce-admin/customers/customer-accounts/manage/login-as-customer){target="_blank"} introducida en la versión de marzo de 2026. La mutación `exchangeOtpForCustomerToken` sigue precisando reCAPTCHA de forma predeterminada cuando reCAPTCHA está habilitado para el inicio de sesión de clientes. Póngase en contacto con el administrador de satisfacción del cliente de Adobe Commerce para activar esta opción. <!-- ACCS-850 -->

### Editar pedidos parcialmente facturados

El botón [!UICONTROL **Editar**] está ahora disponible en la pantalla [!UICONTROL **Vista de pedidos**] para pedidos parcialmente facturados, lo que proporciona a los comerciantes mayor flexibilidad para modificar pedidos que aún están en curso. Anteriormente, no se podían editar los pedidos con facturas, incluso cuando quedaban artículos sin facturar. Siempre que cualquier artículo del pedido se pueda seguir facturando, el pedido se puede editar. Los comerciantes con integraciones personalizadas que dependan de la restricción de edición anterior deben revisar sus flujos de trabajo. <!-- ACCS-849 -->

### Mejoras y correcciones de errores

En esta versión se incluyen las siguientes mejoras, optimizaciones y correcciones de errores seleccionadas:

* Se ha corregido un problema en el cual el atributo de extensión `stock_item` no se devolvía en el extremo de lista de API REST `GET /V1/products`. La respuesta ahora coincide con el contrato de API documentado. <!-- ACCS-819 -->

* Se ha corregido un problema en el cual los vínculos de restablecimiento de contraseña en correos electrónicos devolvían un error 404. <!-- ACCS-797 -->

* Se ha mejorado el rendimiento de las consultas del historial de pedidos para las empresas que utilizan filtros de intervalo de fechas. <!-- ACCS-859 -->

* Se ha corregido un problema en el cual los usuarios de la compañía B2B podían ver solicitudes de igual a igual desde antes de que un usuario se uniera a la compañía. <!-- ACCS-859 -->

* Se han resuelto problemas de tiempo de espera de desprotección que podrían afectar el rendimiento de la API REST al cargar presupuestos con `trigger_recollect` habilitado. <!-- CCSAAS-4904 -->
...
## Abril de 2026: #3 de la versión

[!BADGE Producción]{type=Neutral tooltip="Los elementos enumerados están disponibles actualmente en entornos de producción."}

Los siguientes elementos se lanzaron a los entornos de producción el 27 de abril de 2026.
...
* Se agregó el extremo de la API REST `GET /V1/order-statuses` para recuperar todos los estados de pedidos configurados con sus asignaciones de estado. <!-- CEXT-6100 -->

* Se ha resuelto un problema que provocaba que `custom_attributes` para entidades como Pedido, Carro de compras, Factura, Nota de crédito y Compañía no se mostraran en el esquema REST. <!-- CCSAAS-4818 -->

* Se han resuelto errores de procesamiento de mensajes duplicados (`MessageLockException`) en el consumidor de API masivo y asincrónico. <!-- CCSAAS-4805 -->

* Los atributos de producto de número ahora se representan como filtros de rango de/a en la cuadrícula de producto [!DNL Commerce Admin] cuando el atributo está habilitado para las opciones de filtro. <!-- ACCS-761 -->

* Se corrigió un problema en el cual los correos electrónicos de recordatorio de abandono del carro de compras no mostraban imágenes de productos al usar [!DNL AEM Assets]. <!-- ACCS-798 -->

* Se ha corregido un problema en el cual podía aparecer un error falso de &quot;tamaño máximo de carga&quot; al agregar archivos, muestras o vínculos a productos descargables. <!-- ACCS-813 -->

* Se ha corregido un problema en el cual al guardar un producto asignado a varios catálogos compartidos se podía producir un error. <!-- ACCS-788 -->

* Se ha corregido un problema en el cual la consulta del historial de pedidos podía ser lenta y provocar errores de memoria insuficiente en la base de datos para empresas con muchos pedidos. <!-- ACCS-808 -->

* Se corrigió un problema en el cual la validación del archivo de importación podría fallar. <!-- CCSAAS-4364 -->

* Se ha eliminado la configuración de **[!UICONTROL Recently Viewed/Compared Products]** de la sección **[!UICONTROL Catalog]** en **[!UICONTROL Stores]** > _[!UICONTROL Settings]_>**[!UICONTROL Configuration]**, ya que no es compatible con el administrador de [!DNL Adobe Commerce as a Cloud Service]. <!-- ACCS-793 -->

>[!ENDSHADEBOX]

## Abril de 2026: #2 de la versión

[!BADGE Producción]{type=Neutral tooltip="Los elementos enumerados están disponibles actualmente en entornos de producción."}

<!-- [!BADGE Sandbox]{type=Caution tooltip="The items listed are currently only available in Sandbox environments. Adobe makes new releases available in Sandbox environments first to provide time to test upcoming changes before the release is available on Production environments."} -->

Los siguientes elementos se lanzaron a los entornos de producción el 16 de abril de 2026.

>[!BEGINSHADEBOX]

### Implementar totales de presupuesto personalizados con webhooks

El `plugin.magento.out_of_process_totals_collector.api.get_total_modifications.execute` [gancho web](https://developer.adobe.com/commerce/extensibility/webhooks/) ya está disponible en [!DNL Adobe Commerce as a Cloud Service]. Utilícela para implementar modificaciones de totales de presupuestos personalizados mediante la extensibilidad fuera de proceso. <!-- CEXT-5896 -->

### Reglas de recordatorio de correo electrónico reutilizables (experimental)

>[!IMPORTANT]
>
>Esta función es experimental y debe habilitarse poniéndose en contacto con el administrador de éxito del cliente de Adobe Commerce o creando un ticket de asistencia.

[Las reglas de recordatorio de correo electrónico](https://experienceleague.adobe.com/es/docs/commerce-admin/marketing/communications/email-reminders/email-reminder-rules#rule-repeatability) ahora admiten una configuración opcional de reutilización de reglas que permite que la misma regla se vuelva a aplicar a un cliente después de que ya no se aplique la condición de déclencheur original.

Por ejemplo, si un cliente abandona un carro de compras, completa la compra y posteriormente abandona un carro de compras nuevo, la regla puede almacenar déclencheur de nuevo. Sin esta configuración, un cliente que borra la déclencheur original se excluye permanentemente de futuras coincidencias de la misma regla.

### Ver el informe Transacciones de Servicios de Pago

Si tiene [[!DNL Payment Services]](https://experienceleague.adobe.com/es/docs/commerce/payment-services/get-started/production) habilitado, la [interfaz de usuario del panel](../payment-services/payments-home.md) ya está disponible en [!DNL Commerce Admin], lo que proporciona acceso al [informe de transacciones](../payment-services/reporting.md#transactions-report-view) para ver y administrar transacciones de pago. <!-- PAY-6510 -->

### Mejoras y correcciones de errores

En esta versión se incluyen las siguientes mejoras, optimizaciones y correcciones de errores seleccionadas:

* Se ha corregido un error 500 en la página de empresas de asignación de catálogos compartidos que se podía producir al usar catálogos compartidos grandes con la interfaz de usuario de administración de SDK. <!-- CCSAAS-4783 -->

* Se ha corregido un problema en el cual los clientes de la compañía no podían ver sus propios pedidos si estos se realizaban antes de que el cliente se asignara a la compañía. <!-- ACCS-746 -->
...
Los siguientes elementos se lanzaron a los entornos de producción el 1 de abril de 2026.

>[!BEGINSHADEBOX]

### Añadir archivos a los productos

[!DNL Adobe Commerce as a Cloud Service] ahora admite [agregar archivos a productos](./product-files.md) mediante atributos de producto de tipo de archivo. Puede cargar archivos manualmente en la página de edición del producto, mediante programación a través de la API de REST o de forma masiva proporcionando direcciones URL externas en CSV. <!-- ACCS-535, ACCS-565 -->

### Comprobación del estado de suscripción de alertas de precios y cotizaciones mediante GraphQL

Las nuevas consultas de GraphQL, [`isSubscribedProductAlertStock`](https://developer.adobe.com/commerce/webapi/graphql/schema/products/queries/is-subscribed-product-alert-stock){target="_blank"} y [`isSubscribedProductAlertPrice`](https://developer.adobe.com/commerce/webapi/graphql/schema/products/queries/is-subscribed-product-alert-price){target="_blank"}, permiten a las tiendas determinar si un comprador ya está suscrito a las alertas de existencias o precios de un producto. <!-- ACCS-334 -->

### Crear atributos de producto numéricos que admitan valores negativos

Un nuevo `numeric` [tipo de entrada de atributo de producto](https://experienceleague.adobe.com/es/docs/commerce-admin/catalog/product-attributes/attributes-input-types) permite a los comerciantes crear atributos decimales que admitan valores negativos. <!-- ACCS-600 -->

### Consultar la configuración de reCAPTCHA para varios formularios en una solicitud de GraphQL

La consulta [`recaptchaFormConfigs`](https://developer.adobe.com/commerce/webapi/graphql/schema/store/queries/recaptcha-form-configs) puede devolver detalles de configuración para varios tipos de formulario en una única solicitud. <!-- ACCS-628 -->

### Ver todos los pedidos de la compañía con un nuevo permiso B2B

Un nuevo `view_all_company_orders` [rol de compañía](https://developer.adobe.com/commerce/webapi/rest/b2b/roles) permite a los clientes de empresa B2B ver todos los pedidos dentro de su compañía, incluidos los pedidos históricos creados por usuarios administradores. <!-- ACCS-675 -->

### Mejoras y correcciones de errores

En esta versión se incluyen las siguientes mejoras, optimizaciones y correcciones de errores seleccionadas:

* Ahora puede filtrar los resultados de la API REST de pedidos y empresas utilizando atributos personalizados aplicables, admitiendo escenarios como búsquedas de pedidos con alcance de empresa. <!-- ACCS-633 -->

* Se ha resuelto un error que podía aparecer en la consola del desarrollador del explorador. <!-- CCSAAS-4650 -->

* Se ha corregido un error que se podía producir al cancelar un pedido de invitado con comentarios de pedido. <!-- ACCS-674 -->

* Se ha corregido un error que se podía producir al agregar un producto agrupado con muchos artículos asociados a una lista de solicitudes. <!-- ACCS-627 -->

>[!ENDSHADEBOX]

## Marzo de 2026: #2 de la versión

[!BADGE Producción]{type=Neutral tooltip="Los elementos enumerados están disponibles actualmente en entornos de producción."}

Los siguientes elementos se lanzaron a los entornos de producción el 24 de marzo de 2026.

>[!BEGINSHADEBOX]

### Iniciar sesión como cliente utilizando códigos de un solo uso

Los administradores ahora pueden generar [códigos de una sola vez](https://experienceleague.adobe.com/es/docs/commerce-admin/customers/customer-accounts/manage/login-as-customer) para la suplantación de clientes mediante la API [!DNL Commerce Admin] y REST. El código de una sola vez se puede intercambiar por un token de acceso de cliente a través de las mutaciones de GraphQL `generateCustomerToken` o `exchangeOtpForCustomerToken`, lo que permite flujos sin contraseña de &quot;Iniciar sesión como cliente&quot; para escenarios de compras asistidas por el vendedor. <!-- ACCS-404 -->

Para obtener instrucciones sobre cómo implementar esta característica mediante API, consulte la [API de REST](https://developer.adobe.com/commerce/webapi/rest/saas-integrations/login-as-customer/) y la [documentación de GraphQL](https://developer.adobe.com/commerce/webapi/graphql/schema/customer/mutations/generate-token).

### Administrar cuentas de tarjetas de regalo mediante la API de REST

Ahora se pueden crear, actualizar, eliminar y consultar [cuentas de tarjeta regalo](https://developer.adobe.com/commerce/webapi/rest/saas-integrations/gift-card-accounts/) a través de la API de REST. Además, la compatibilidad con la importación masiva de JSON está disponible a través del extremo `/V1/import/json`, lo que permite que las integraciones de terceros sincronicen tarjetas regalo mediante programación. <!-- ACCS-476 -->

### Déclencheur de correos electrónicos transaccionales mediante la API de REST

Un nuevo extremo de API de REST (`POST /V1/custom-email/send`) le permite [almacenar en déclencheur los mensajes de correo electrónico transaccionales](https://developer.adobe.com/commerce/webapi/rest/saas-integrations/custom-email/) bajo demanda especificando una ID de plantilla de correo electrónico, un correo electrónico de destinatario y variables de plantilla. La API admite matrices anidadas como variables de plantilla para contenido de correo electrónico complejo. <!-- ACCS-325, ACCS-481 -->
...
Las tiendas EDS ahora funcionan con [alertas de precios y existencias](https://experienceleague.adobe.com/es/docs/commerce-admin/inventory/configuration/product-alerts/alert-setup). <!-- ACCS-334 -->
...
El límite del número de sitios web, tiendas y vistas de tiendas estaba limitado anteriormente a 50. Ahora puede enviar una [solicitud de soporte](https://experienceleague.adobe.com/home?lang=es&support-tab=home#support) para modificar estos límites, si es necesario. <!-- ACCS-398 -->
...
El [módulo de recordatorio de correo electrónico](https://experienceleague.adobe.com/es/docs/commerce-admin/marketing/communications/email-reminders/email-reminder-rules) (`Magento_Reminder`) está ahora activo en [!DNL Adobe Commerce as a Cloud Service]. Permite a los comerciantes crear reglas de recordatorio automatizadas que almacenan en déclencheur los correos electrónicos enviados a los clientes en función de la inactividad del carro de compras y la lista de deseos. <!-- CCSAAS-4597 -->
...
Una nueva configuración opcional en el nivel de tienda permite a los clientes [hacer un seguimiento de los pedidos de los invitados](https://experienceleague.adobe.com/es/docs/commerce-admin/stores-sales/point-of-purchase/checkout/checkout-guest#allow-guest-order-access-for-registered-emails) que hayan hecho. Esto se aplica si el pedido se realizó usando una dirección de correo electrónico que coincida con una cuenta de cliente registrada. <!-- ACCS-289 -->
...
* Se ha agregado acceso a los informes de administración, incluidos los informes de [Clientes](https://experienceleague.adobe.com/es/docs/commerce-admin/start/reporting/customer-reports), [Marketing](https://experienceleague.adobe.com/es/docs/commerce-admin/start/reporting/marketing-reports), [Productos](https://experienceleague.adobe.com/es/docs/commerce-admin/start/reporting/product-reports) y [Ventas](https://experienceleague.adobe.com/es/docs/commerce-admin/start/reporting/sales-reports). <!-- CCSAAS-3085 -->
...
La API de facturas ahora admite [importes de captura personalizados](https://experienceleague.adobe.com/es/docs/commerce-admin/stores-sales/order-management/invoices#custom-capture-amounts) mediante atributos de extensión. <!-- ACCS-186, ACCS-197, ACCS-143 -->
...
Ahora puede [asignar acceso de usuarios](./user-management.md#add-users) a instancias individuales de [!DNL Adobe Commerce as a Cloud Service] en Admin Console. <!-- CCSAAS-4337 -->
<!-- See PR #332 -->
...
* Se ha resuelto un problema en el cual los atributos personalizados del producto no se actualizaban a través de la API de REST cuando [!DNL AEM Assets integration] estaba habilitado y el producto tenía imágenes. <!-- ACAP-1178 -->

* Varias mejoras de rendimiento y optimización.
<!-- CCSAAS-4255 -->
<!-- CCSAAS-4233 -->
<!-- CCSAAS-4220 -->
<!-- CCSAAS-4252 -->
